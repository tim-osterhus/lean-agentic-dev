...
2) Clarify what was requested:
   ...
   - What constraints apply (from `AGENTS.md`)?
...
7) Execute validation:
   ...
   - Verify constraints from `AGENTS.md` are still satisfied.
...
9) If everything aligns:
   - Append a confirmation to `agents/historylog.md`.
...
10) If gaps exist:
    ...
    - Append QA entry to `agents/historylog.md` noting gaps.
...
- `agents/historylog.md` must include a QA entry (success or gaps).
